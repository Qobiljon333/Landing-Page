      ...
      <div className=" flex flex-col pl-16 pb-16   flex-wrap lg:col-span-6 sm:h-[850px] 2xl:h-[50vh] ">
        {
          COUNTRY.map(({name,countrys},inx)=>(
            <div className=' hover:cursor-pointer' key={inx}>
              ...
              } className={`flex my-2   items-center   h-[50px] ${state && letter === name? "h-auto": "max-h-[40px]"}`}><h1 className='text-2xl  mr-7'>{name}</h1> <MdOutlineArrowRight className={` ${state && letter === name ? "rotate-90":""} transitionn text-2xl`}/>  </div>
              <div className={`${state && letter === name? "h-auto": "h-0"} overflow-hidden`}>
                {
                  countrys.map((country)=>(
                    <p  className={`text-xl hover:cursor-pointer  transition-all ml-8 my-2 ${ letter === name? "scale-100 opacity-100": "scale-0 opacity-0"}`}>{country}</p>
                  ))
                }
              </div>
            </div>
